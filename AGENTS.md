...
**Blog URL pattern is SEO-critical and fixed.** All 44 articles must be served at `/blog/f/[slug]`. This matches legacy GoDaddy URLs indexed in search engines. Never simplify to `/blog/[slug]`. The canonical slug list lives in `src/lib/blog.ts` → `CARRY_FORWARD_SLUGS`.
...
src/content/blog/*.md     44 article files with frontmatter
...
**Note (PP2 Session 1, authored 2026-05-11):** The site-root `public/AGENTS.md` (deployed at `https://lanebelone.com/AGENTS.md`) is authored and live, per Web Strategy Codex v1.1.0 Section V.4.b. It is distinct from this repo-root AGENTS.md, which serves AI coding agents working on the codebase. The live file carries the lanebelone-specific content per the codex per-site table: article taxonomy, blog index pattern, Substack canonical pair pattern, content license boundary, plus the all-sites required entries (site purpose statement, content license boundary, pointers to `llms.txt`, `llms-full.txt`, `sitemap.xml` and primary content directories). See `public/AGENTS.md` for the live instructions and `infinitegameos.io/public/AGENTS.md` for the IGOS exemplar.
