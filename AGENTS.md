...
**Blog URL pattern is SEO-critical and fixed.** All 47 articles must be served at `/blog/f/[slug]`. This matches legacy GoDaddy URLs indexed in search engines. Never simplify to `/blog/[slug]`. The canonical slug list lives in `src/lib/blog.ts` → `CARRY_FORWARD_SLUGS`.
