// Sovereign Email Preference Center, Phase 1. List-aware resubscribe and join.
// Adds the tag to an existing contact, clears the global flag, unsuppresses that
// audience. No welcome email, this is a restore at the data layer. Verifies the
// deterministic HMAC token. Mirror across lanebelone and IGOS.

import { NextRequest, NextResponse } from 'next/server'
import { verifyAnyToken } from '@/lib/unsubscribe-token'
import { applyListResubscribe } from '@/lib/email-lists'
...
export async function POST(req: NextRequest) {
  ...
  const result = await applyListResubscribe({ email, listKey: list, ip })
